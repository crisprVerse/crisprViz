...
-> start describing scenario
-> mention how can add annotation tracks
-> one track of interest is repeats, which we want to avoid as they can lead to many off-targets
-> will add repeats annotation
...
-> Repeats overlaps some gRNAs in our target region. Let's remove them with removeRepeats()
-> regenerate plot to see they were removed
...
-> We can do the same with another type of genomic annotation we wish to avoid: SNPs. However, rather than adding SNPs as a separate annotation, we can incorporate SNPs into our `GuideSet` with addSNPAnntation(), and plot a SNPs track with `includeSNPTrack=TRUE`, which is the default value. Note that since SNPs are obtained from the GuideSet, only SNPs that overlap with gRNAs in the GuideSet will be plotted. addSNPAnnotation() also appends a logical `hasSNP` column which we could use to filter out guides overlaping SNPs.

-> Some annotations we are interested in are CAGE peaks and DNase (briefly, why), let's retrieve those annotations from AnnotationHub and plot them alongside our Guides
...
## h3k27 track?
# gc-content: data track - need to create
...
-> we can zoom in on annotated regions, as this is a more desirable target location, and add on-target scores
-> we have fewer choices in this range for SpCas9, let's add another GuideSet for AsCas12a to compare the number and quality of hits; (we can also add a dataTrack for GC content...)
